        ...
        app.delete("/deleteCard/:id", async (req, res) => {
            try {
                ...
                if (result.deletedCount === 0) {
                    return res.status(404).send("No robot with the specified ID found");
                }
        
                res.status(200).send("Robot deleted successfully");
            } catch (error) {
                console.error("Error occurred in DELETE:", error);
                res.status(500).send("Error occurred while deleting the robot");
            }
        });    
